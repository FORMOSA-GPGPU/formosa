...
local system = require("kernel-sim.system")("vecmul.lua", { ... })
...
for i = 0, n - 1 do
  a[i] = math.random(-1000000, 1000000)
  b[i] = math.random(-1000000, 1000000)
end
...
ksim.run_kernel(
  system,
  util.runfile("bin/vecmul.kernel-sim.elf"),
  kargs_addr,
  n,
  system.num_threads
)
...
-- Download the result.
local c_result = ffi.new("long long[?]", n)
