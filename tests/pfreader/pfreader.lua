...
local cpu = cp.CommandProcessor("cp", 0, { core_trace = false })

local mem_size = 0x200000
local cp_csr_size = 0x38
local cp_csr_base = mem_size + 0x8 * 4 + 0x4
...
local xbar = simple.XBar("xbar", 2, {
  ...
  { addr = mem_size, size = 0x8 * 3 },
  { addr = mem_size + 0x8 * 3, size = 0x4 },
  { addr = cp_csr_base, size = cp_csr_size },
})

local printbuf = simple.PrintBuf("printbuf", 1)
local cp_csr = simple.Memory("cp_csr", { size = cp_csr_size })

local pfreader = simple.PerfettoReader("PerfettoReader", {
  { type = 0, track_name = "SliceTrack", content_name = { "First slice", "Second slice" } },
  { type = 1, track_name = "InstantTrack", content_name = { "First instant", "Second instance" } },
  { type = 2, track_name = "CounterTrack" },
})
...
xbar.mem_side[2].target = pfreader.slave_port
xbar.mem_side[3].target = printbuf.port
