...
float *OpenClFindNearestNeighbors(cl_context context, int numRecords,
  ...
  if (timing) {
    ...
    printf(
        "# Records\tWrite(s) [size]\t\tKernel(s)\tRead(s)  "
        "[size]\t\tTotal(s)\n");
    printf("%d        \t", numRecords);
    // Write Buffer
...
int loadData(const std::string &filename, std::vector<Record> &records,
             std::vector<LatLong> &locations) {
  FILE *flist, *fp;
  int i = 0;
  char dbname[64];
  ...
  while (!feof(flist)) {
    ...
    std::string temp(KERNEL_PATH);
    dbpath = temp.substr(0, temp.length() - sizeof("kernel.cl") + 1) + dbpath;
    printf("loading db: %s\n", dbpath.c_str());
    fp = fopen(dbpath.c_str(), "r");
    if (!fp) {
...
int parseCommandline(int argc, char *argv[], std::string *filename, int *r,
...
void printUsage() {
  ...
  printf("\n");
  printf(
      "-p [int]     Choose the platform (must choose both platform and "
      "device)\n");
  printf(
      "-d [int]     Choose the device (must choose both platform and "
      "device)\n");
  printf("\n");
  printf("\n");
