...
static int initialize(int use_gpu) {
  ...
  result =
      clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_DEFAULT, 1, device_list, NULL);
...
int allocate(int n_points, int n_features, int n_clusters, float **feature) {
  ...
  err = read_kernel_file(KERNEL_PATH, &kernel_bin, &kernel_size);
  if (err != CL_SUCCESS) {
    printf("ERROR: read_kernel_file() => %d\n", err);
    return -1;
  }
  prog = clCreateProgramWithSource(context, 1, (const char **)&kernel_bin,
                                   &kernel_size, &err);
