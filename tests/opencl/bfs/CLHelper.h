...
free(allPlatforms);*/
 ...
 if (deviceListSize == 0)
  ...
  if ((resultCL != CL_SUCCESS) || (oclHandles.program == NULL)) {
    ...
    if (resultCL != CL_SUCCESS)
      throw(
          string("InitCL()::Error: Getting Program build "
...
// get program information in intermediate representation
  ...
  for (int i = 0; i < deviceListSize; i++) binaries[i][binary_sizes[i]] = '\0';
  ...
  for (int i = 0; i < deviceListSize; i++) free(binaries[i]);
#endif
...
void _clMemcpyD2H(cl_mem d_mem, int size, void *h_mem) {
  ...
  if (oclHandles.cl_status != CL_SUCCESS) throw(oclHandles.error_str);
#endif
}
...
void _clInvokeKernel(int kernel_id, int work_items, int work_group_size) {
  ...
  // cl_event e[1];
  if (work_items % work_group_size !=
      0)  // process situations that work_items
...
void _clFree(cl_mem ob) {
  if (ob != NULL) oclHandles.cl_status = clReleaseMemObject(ob);
#ifdef ERRMSG
