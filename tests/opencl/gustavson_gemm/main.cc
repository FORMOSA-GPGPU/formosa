...
void vector_random(std::vector<T> &vec) {
  const int sparsity_threshold = 80;  // 80% threshold for sparse matrix

  for (int i = 0; i < (int)vec.size(); i++) {
    // Random generate 0 with sparsity_threshold
    if ((rand() % 100) < sparsity_threshold) {
      vec[i] = 0;
    } else {
      // Generate value without 0
      if constexpr (std::is_integral_v<T>) {
        vec[i] = (rand() % 100) + 1;
      } else if constexpr (std::is_floating_point_v<T>) {
        vec[i] = ((rand() % 100) / 10.0) + 0.1;
      }
    }
  }
}
...
bool build_and_test(cl::CommandQueue &queue, cl::Context &context,
  ...
  // Set the kernel
  cl::Kernel gustavson_gemm(program, "gustavson_gemm");

  int k = K;
  int n = N;
  int wg_size = WG_SIZE;

  gustavson_gemm.setArg(0, buf_A);
  gustavson_gemm.setArg(1, buf_B);
  gustavson_gemm.setArg(2, buf_C);
  gustavson_gemm.setArg(3, k);
  gustavson_gemm.setArg(4, n);

  gustavson_gemm.setArg(5, sizeof(T) * N, nullptr);

  // 1 dimension execution, every thread deal with one row
  // Execute the kernel
  cl::NDRange localSize(wg_size);
  cl::NDRange globalSize(M * wg_size);
  queue.enqueueNDRangeKernel(gustavson_gemm, cl::NullRange, globalSize,
                             localSize);
  // Read the result
  ...
  if (has_error) {
    std::cerr << RED << "Gustavson_Gemm " << type_name << " Failed!" << RESET
              << std::endl;
    return false;
  } else {
    std::cout << GREEN << "Gustavson_Gemm " << type_name << " Passed!" << RESET
              << std::endl;
    return true;
  }
}
...
int main(int argc, char **argv) {
  ...
  passed &=
      build_and_test<float>(queue, context, program, "float", (float)FP_ERROR);
  passed &= build_and_test<double>(queue, context, program, "double",
                                   (double)DP_ERROR);
