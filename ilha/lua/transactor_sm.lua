...
---@class ilha.transactor_sm.config
---@field sys_config table

---@class ilha.transactor_sm : ilha.system.sm
---@field protected _clock sc.clock
---@field protected _sc_module sc.Module
---@field protected _transactor ilha.SMTransactor
---@field protected _dummy_initiator simple.Initiator
---@overload fun(name: string, config: ilha.transactor_sm.config, clock: sc.clock, reset_n: sc.signal, id: integer): ilha.transactor_sm
local TransactorSM = {}

---@param name string
---@param config ilha.transactor_sm.config
---@param clock sc.clock
---@param reset_n sc.signal
---@param id integer
---@return ilha.transactor_sm
function TransactorSM.new(name, config, clock, reset_n, id)
  assert(config and config.sys_config, "config.sys_config is required")
  ---@type ilha.transactor_sm
  local self = setmetatable({}, TransactorSM --[[@as table]])
  self._clock = clock
  self._transactor = ilha.SMTransactor("transactor", {})
