...
---@overload fun(name: string, config: ilha.config, opts: ilha.system.opts): ilha.system
local System = {}
System.__index = System

---@param name string
---@param config ilha.config
---@param opts ilha.system.opts
---@return ilha.system
-- `name` is consumed by lv.sc_module.wrap to name the backing SystemC module.
function System.new(name, config, opts)
  config:validate()
  ...
  self._period = sc.time(system.clock_period_ns, sc.time_unit.NS)
  self._clock = sc.clock("clock", self._period)
  self._rst_n = sc.signal("rst_n", true)
  self._gnd = sc.signal("gnd", false)

  -- Fixed address-map apertures (see ilha.addr_map / generated formosa_addr_map.h)
  local addr = require("ilha.addr_map")
  local rom_size = 0x200
  local cp_tcm_base = addr.cp_tcm_base
  local cp_tcm_size = addr.cp_tcm_size
  local clint_size = addr.clint_size
  local clint_base = addr.clint_base
  local host_dma_csr_base = addr.host_dma_csr_base
  local host_dma_csr_size = addr.host_dma_csr_size
  local device_dma_csr_base = addr.device_dma_csr_base
  local device_dma_csr_size = addr.device_dma_csr_size
  local cp_csr_size = addr.cp_csr_size
  local onchip_gmem_base = addr.onchip_gmem_base
  local onchip_gmem_size = addr.onchip_gmem_size
  local scratch_base = addr.scratch_base
  local scratch_size = addr.scratch_size
  local cmd_ring_base = addr.cmd_ring_base
  local max_size = addr.max_size
  local shared_cache_size = system.shared_cache_size
  local sm_mmio_base = addr.sm_mmio_base
  local sm_mmio_stride = addr.sm_mmio_stride
  local cp_printbuf_base = addr.cp_printbuf_base
  local cp_exit_base = addr.cp_exit_base
  local cp_pfreader_base = addr.cp_pfreader_base
  local max_pfreader_tracks = 7
    + addr.cp_kernel_state_buf_size
    + addr.sm_mmio_aperture / addr.sm_mmio_stride * (1 + system.wg_resident_limit)
  local pfreader_reserved_size = 0x8 * max_pfreader_tracks

  assert(system.num_sm * sm_mmio_stride <= addr.sm_mmio_aperture)
  -- pfreader sits in CP_CTRL; keep reserved max size inside the aperture path to SM MMIO
  assert(cp_pfreader_base + pfreader_reserved_size <= addr.sm_mmio_base)
  assert(cp_pfreader_base + pfreader_reserved_size <= host_dma_csr_base)
  assert(device_dma_csr_base + device_dma_csr_size <= cp_tcm_base)
  assert(addr.fsa_mmio_base + addr.mmio_size <= sm_mmio_base)

  -- Unified scratch SRAM covers CP CSR through the ABI scratch region.
  local scratch_mem_base = addr.cp_csr_base
  local scratch_mem_end = scratch_base + scratch_size
  local scratch_mem_size = scratch_mem_end - scratch_mem_base
  assert(addr.cp_csr_base + cp_csr_size <= scratch_mem_end)
  assert(scratch_mem_size > 0)

  -- ABI v3 scratch: command ring first at scratch base, then completion pool.
  local ring_bytes = addr.cmd_ring_entries * addr.cmd_packet_size
  local pool_bytes = addr.completion_pool_bytes
  assert(pool_bytes ~= nil and pool_bytes > 0, "completion_pool_bytes required")
  assert(scratch_size >= ring_bytes + pool_bytes)
  assert(cmd_ring_base % 64 == 0, "cmd_ring_base must be 64-byte aligned")
  assert(cmd_ring_base == scratch_base, "command ring must start at scratch base")
  assert(
    addr.completion_pool_base == cmd_ring_base + ring_bytes,
  ...
  for i = 0, addr.cp_kernel_state_buf_size - 1 do
  ...
  local wgi_buf_count = system.num_sm * system.wg_resident_limit
  ...
  self._cp_tcm = simple.Memory("cp_tcm", { size = cp_tcm_size })
  ...
  self._fab_cp = simple.XBar("fab_cp", 1, {
    ...
    { addr = cp_printbuf_base, size = 0x8 }, -- printbuf
    { addr = cp_exit_base, size = 0x8 }, -- exit code register
    { addr = cp_pfreader_base, size = pfreader_reserved_size }, -- pfreader (max tracks)
    { addr = host_dma_csr_base, size = host_dma_csr_size },
    { addr = device_dma_csr_base, size = device_dma_csr_size },
    { addr = cp_tcm_base, size = cp_tcm_size }, -- TCM
    {
      addr = clint_base,
      size = max_size - clint_base,
  ...
  local fab_nexus_mmap = {
    {
      addr = clint_base,
      size = (addr.fsa_mmio_base + addr.mmio_size) - clint_base,
      subtract_start_addr = false,
    }, -- agent (CLINT + scratch)
  }
  for i = 0, system.num_sm - 1 do
  ...
  self._system_info = simple.ConstantTable("SystemInfo", {
    entries = {
      { addr = addr.system_info_off_abi_version, size = 8, value = addr.system_info_version },
      { addr = addr.system_info_off_length, size = 8, value = addr.system_info_length },
      { addr = addr.system_info_off_num_sm, size = 8, value = system.num_sm },
      { addr = addr.system_info_off_threads_per_warp, size = 8, value = system.threads_per_warp },
      { addr = addr.system_info_off_warps_per_core, size = 8, value = system.warps_per_core },
      { addr = addr.system_info_off_local_mem_size, size = 8, value = addr.lmem_size },
      { addr = addr.system_info_off_shared_cache_size, size = 8, value = system.shared_cache_size },
      { addr = addr.system_info_off_cache_block_size, size = 8, value = system.cache_block_size },
    },
  })
  self._agent = ipc.Agent("agent", {
    socket_path = opts.agent_socket_path,
    timeout_ms = 100,
    debug = false,
  ...
  local csr_off = addr.cp_csr_base - scratch_mem_base
  ...
  self._scratch:write_bytes(csr_off + 0x28, u64_le(addr.cmd_ring_entries)) -- CMD_RING_SIZE
  local fab_agent_mmap = {
    { addr = clint_base, size = clint_size }, -- CLINT (reboot + timer)
  ...
  self._dram = dramsys.DRAMSys("dram", {
    config = system.dram_config,
  })
  -- fab_sys masters: host_dma.port1, nexus, SMs, device_dma.port0, device_dma.port1
  self._fab_sys = simple.XBar("fab_sys", 4 + system.num_sm, {
    { addr = onchip_gmem_base, size = onchip_gmem_size }, -- on-chip GMEM
  ...
  self._fab_cp.mem_side[4].target = self._cp_pfreader.slave_port
  self._fab_cp.mem_side[5].target = self._host_dma.slave_port
  self._fab_cp.mem_side[6].target = self._device_dma.slave_port
  self._fab_cp.mem_side[7].target = self._cp_tcm.port
  self._fab_cp.mem_side[8].target = self._fab_nexus.core_side[1].port
