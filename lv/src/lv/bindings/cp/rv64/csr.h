...
struct csr_misa {
  ...
  bool has_C_extension() { return fields.extensions & C; }

  bool has_user_mode_extension() { return fields.extensions & U; }

  bool has_supervisor_mode_extension() { return fields.extensions & S; }
...
struct csr_pmpaddr {
  ...
  unsigned get_address() { return fields.address << 2; }
};
...
/* Actually 32 bit large, but use 64 value for consistency and simply set the
 * read/write mask accordingly. */
struct csr_fcsr {
  union {
    uint64_t reg = 0;
    struct {
